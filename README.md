...
Моя реализация находится по [этой ссылке](TemplateOfDealsViewer/ViewController.swift) и запускается по дефолту. Для желающих решить тестовое самостоятельно прикладываю условие задания + template ViewController, для запуска нужно изменить начальный ViewController на TemplateViewController
